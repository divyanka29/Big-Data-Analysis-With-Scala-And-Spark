...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val groups: Map[Int, List[Column]] = columnNames
      .foldLeft(List.empty[(Int, Column)])((acc, name) => {
        distinctCategories
          .flatMap {
            case (prefix, index) if prefix.exists(name.startsWith) => Some(index, new Column(name))
            case _ => None
          }
          .sortBy(_._1)
          .headOption match {
    ...
    val output = (0 to 2).map(index => groups.getOrElse(index, List.empty[Column]))
  ...
  def timeUsageGroupedSql(summed: DataFrame): DataFrame = {
    val name = s"summed"
    summed.createOrReplaceTempView(name)
    spark.sql(timeUsageGroupedSqlQuery(name))
  }
